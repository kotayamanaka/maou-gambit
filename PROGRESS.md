...
## 2026-06-26 高価値捕獲の搬送優先

### 目的

`牢屋搬送` を持つ配下でも、同じ部屋に生存敵がいる間は攻撃が優先され、高価値なダウン敵や残り時間の短いダウン敵を拾い遅れることがあった。捕獲が本作の中核なので、チップを渡した意味が戦闘中に出るよう、高価値/緊急のダウン体だけは攻撃継続より先に回収する。

### 修正

- `decideAllyAction` に緊急ダウン体判定を追加した。
- `牢屋搬送` を持ち、同じ部屋に捕獲難度4以上かつ残り7秒以下のダウン敵がいる場合は、生存敵がいても先に拾いに行く。
- 通常のダウン敵は従来どおり、攻撃を片付けてから拾う余地を残す。
- 複数のダウン敵がいる場合は、捕獲難度、残り時間、距離を見て優先対象を選ぶようにした。
- 初回バランス確認で、捕獲難度3以上や通常敵の残り5秒以下を緊急扱いにすると序中盤で戦線を空けすぎたため、緊急対象を賢者/勇者級へ絞った。

### スクリーンショット

- `screenshots/capture-priority-battle-desktop.png`

### 検証結果

- `npx playwright test tests/smoke.spec.js -g "carrier prioritizes urgent|carrier returns" --reporter=line`: PC/スマホ 4件成功。
- Playwright PC幅で、生存中の戦士が同室にいても、ゴブリンが高難度の賢者ダウン体を先に担ぐ状態を確認した。
- `npm run test:balance`: 成功。緊急判定調整後は Raw stage winRate 0.55、avgCaptured 1、campaign は20ステージ勝利。
- `npm test -- --reporter=line`: 96件成功。
- `npm run build:pages`: 成功。

### 残課題

- 捕獲成立頻度はまだステージ、部屋距離、運搬役の配置に左右される。高価値回収の成立率を見ながら、ダウン猶予、運搬速度、牢屋位置のバランスを続けて調整する。
- 今後、チップ順序や条件チップをよりデータ駆動化すると、捕獲役/攻撃役/足止め役の作り分けがさらに明確になる。

## 2026-06-26 部屋建設プランUI

### 目的

部屋建設はドラッグ配置、配置点、接続元、扉方向、曲がる通路プレビューまで入っているが、プレイヤー視点では「この部屋がどこからどの扉でつながるのか」を建設直前に読み取りづらかった。ダンジョンを自分で拡張している感覚を強めるため、配置点に応じた自然な扉方向と建設プラン確認を追加する。

### 修正

- 建設パネルに `建設プラン` を追加し、接続元、接続元の扉、建設先、建設先の自動扉、方角/距離、建設可否を一行で確認できるようにした。
- `selectedBuildDoor` の初期値を固定の北扉から自動推奨へ変更した。
- 配置点と建設部屋から接続元の推奨扉を算出し、未指定時は自然に配置点側へ通路が伸びるようにした。
- 接続扉レーンに `自動` を追加し、手動で北/東/南/西を選んだあとでも自動推奨へ戻せるようにした。
- 手動扉ボタンには、現在の配置点に対する `推奨` / `手動指定` を表示するようにした。
- スマホ確認幅では建設プランを縦積みにし、パネル内で文字が潰れないようにした。

### 検証結果

- `npx playwright test tests/smoke.spec.js -g "corridors use orthogonal door segments and build slots" --reporter=line`: PC/スマホ 2件成功。
- Playwright PC幅で `screenshots/build-plan-desktop.png` を生成し、`広間 東扉 → 宝物庫西扉` と建設可否が読めることを確認した。
- `npm test -- --reporter=line`: 94件成功。
- `npm run test:balance`: 成功。Raw stage winRate 0.5、campaign は20ステージ勝利。
- `npm run build:pages`: 成功。

### 残課題

- 建設位置は自由ドロップも可能だが、完全なグリッドエディタや通路の曲げ位置指定までは未実装。
- 部屋撤去コスト、接続上限、敵導線の面白さは、20ステージ全体のバランスを見ながら継続調整する。
