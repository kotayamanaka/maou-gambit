...
- 魔王は `assets/generated/characters/demonLord/source-v1.png` を生成原本にし、クロマキー除去後の `demonLord-alpha-v1.png` から `public/assets/sprites/demonLord/idle-front.png` へ縮小・余白調整して使う。現時点の4方向/歩行/攻撃/ダウンは `scripts/make_demon_lord_sprites.py` で既存正面絵から派生生成する。
...
- `demonLord` は専用生成素材から派生した4方向/歩行/攻撃/ダウンのスプライトセットを参照する。
